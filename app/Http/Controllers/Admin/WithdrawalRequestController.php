...
class WithdrawalRequestController extends Controller
{
    ...
    {
        $query = WithdrawalRequest::query()->join('users', 'withdrawal_requests.user_id', '=', 'users.id')->select('withdrawal_requests.*', 'users.name');
    ...
    public function withdrawalAction($id, Request $request)
    {
        ...
        if($request->action == 'approve') {
            ...
            $user->earned_balance -= $withdrawal_request->amount;
            $user->save();
