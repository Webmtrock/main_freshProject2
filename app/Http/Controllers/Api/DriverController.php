        ...
        try {
            ...
            if($userOrder->driver_id == $userOrder->vendor_id) {
                ...
                if($order_detail_save && $driver_user_balance_save) {
                    $this->createWalletTransaction($userOrder->vendor_id, $user_type = 'D', $userOrder->driver_id, $previous_earned_balance, $userOrder->driver->earned_balance, $userOrder->commission_driver, $userOrder->id, 'E', 'Earned form Order Delivery, Order id'.$userOrder->id);
                }

                if(isset($userOrder->tip_amount) && ($userOrder->tip_amount > 0)) {
                    $previous_earned_balance_new = $userOrder->driver->earned_balance;
                    $userOrder->driver->earned_balance += $userOrder->tip_amount;
                    $driver_user_balance_save_new = $userOrder->driver->save();
                    
                    if($order_detail_save && $driver_user_balance_save_new) {
                        $this->createWalletTransaction($userOrder->vendor_id, $user_type = 'D', $userOrder->driver_id, $previous_earned_balance, $userOrder->driver->earned_balance, $userOrder->tip_amount, $userOrder->id, 'E', 'Earned Tip form Order Delivery, Order id'.$userOrder->id);
                    }
                }
                
                $vendor_previous_earned_balance = $userOrder->vendor->earned_balance;
                $earned_balance_from_order = $userOrder->grand_total - $userOrder->commission_driver - $userOrder->commission_admin - $userOrder->tax - $userOrder->tip_amount;
